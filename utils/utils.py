...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    """
    this method will draw a bounding box around the detected plate 
    if a label was provide it will but a label above the box 

    Returns:
        1- image copy after dawring the bounding box and the label if exists 
        2- the top-left and bottomn-right vertices of the bounding box 
    """
    #! notice using img = img.copy() won't work because "img" is a view of the input image
    #! and doing so will also draw on the original input image
    img_copy = img.copy()
    line_thickness = line_thickness or round(
        0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]

    #! vertices of bounding rectaangel for the licence plate
    tl, br = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))

    cv2.rectangle(img_copy, tl, br, color, thickness=line_thickness,
                  lineType=cv2.LINE_AA)
    if label:
        font_sickness = max(line_thickness - 1, 1)  # font thickness
        t_size = cv2.getTextSize(
            label, 0, fontScale=line_thickness / 3, thickness=font_sickness)[0]

        c1 = tl
        c2 = (c1[0] + t_size[0], c1[1] - t_size[1] - 3)
        cv2.rectangle(img_copy, c1, c2, color, -1, cv2.LINE_AA)  # filled

        cv2.putText(img_copy, label, (c1[0], c1[1] - 2), 0, line_thickness / 3,
                    [225, 255, 255], thickness=font_sickness, lineType=cv2.LINE_AA)

    return img_copy, tl, br
